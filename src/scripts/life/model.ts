import { BANDS, COUNTRIES, GETTING_READY, type CountryId } from "./data";

/**
 * What's left of a life: the years someone has left (their country's life table), split up by how people
 * spend an average day at each age from here on (the time use survey), with their screen time carved out
 * of their free time. Used by the page (src/scripts/life.ts) and, for its figures, the home page.
 */
...
export interface Day {
  /** Hours of each part: they add up to 24. */
  hours: Split;
  /** Free time before screens take their share: leisure, sport, volunteering and religion. */
  freeTime: number;
  /** The average screen time for this age. */
  averageScreens: number;
  /** Screen time that wouldn't fit in free time, so has to come out of meals, chores, work or sleep. */
  overflow: number;
}
...
export function dayAt(age: number, screens: number | null): Day {
  ...
  const averageScreens = band.tv + band.gamesComputer;
  const wanted = screens ?? averageScreens;
  const onScreens = Math.min(wanted, freeTime);
  return {
    hours: { sleep, work, upkeep, screens: onScreens, free: freeTime - onScreens },
    freeTime,
    averageScreens,
    overflow: wanted - onScreens,
  };
}
...
export function lifeOf(inputs: Inputs): Life {
  ...
  for (let year = age; year < end; year++) {
    ...
    const { hours } = dayAt(year, screens);
    for (const part of PARTS) years[part] += hours[part] * share;
  }
  return { inputs, left, end, years, today: dayAt(age, screens) };
}
